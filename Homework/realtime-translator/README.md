...
- 답변이 필요할 때 **`Ctrl+Space`**(또는 우측 상단 **답변 추천** 버튼) → 영어 답변 3개. "자동 답변" 토글 ON 시(기본값 OFF)에는 확정 문장마다 자동 호출됩니다.

---

## 저지연 설계 / 속도 튜닝

`.env` 또는 `config.py`에서 조정 가능한 지연 관련 설정:

| 항목 | 빠르게 | 정확하게 |
|---|---|---|
| `WHISPER_MODEL` | `tiny` / `base` | `small` |
| GPU | NVIDIA GPU면 자동 `cuda` 사용 (가장 큰 효과), 실패 시 CPU(int8) 폴백 | - |
| `PARTIAL_INTERVAL` | 낮추면 자막 더 자주 갱신 (기본 0.6초) | 높이면 부하↓ |
| `SILENCE_HANG_SEC` | 낮추면 문장 더 빨리 확정 (기본 0.45초) | 높이면 끊김↓ |

구조적 지연 최소화 요소:

- STT는 로컬 `faster-whisper` 실행이라 네트워크 왕복이 없음
- 오디오 캡처 → STT → 번역이 별도 스레드 + 큐로 연결되어 한 단계가 다음 단계를 블로킹하지 않음
- WASAPI 루프백을 16kHz mono로 다운샘플링해 STT 입력 부하를 낮춤
- 무음 임계치 `SILENCE_RMS`는 환경 소음에 맞춰 조정하세요(자막이 안 뜨면 낮춤)

> **실측 벤치마크는 수행하지 않았습니다.** 위 항목은 설계상 지연을 낮추는 요소들이며, GPU 유무·발화 길이·모델 크기에 따라 실제 체감 속도는 달라질 수 있습니다.
